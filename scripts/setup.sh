...
log ""
log "Run the two development processes in separate terminals:"
log "  make -C backend dev      (API on http://localhost:${PORT:-8080})"
log "  cd frontend && bun run dev   (app on http://localhost:${VITE_PORT:-5173})"
log ""
log "Verify the whole stack with ./scripts/smoke-test.sh"
