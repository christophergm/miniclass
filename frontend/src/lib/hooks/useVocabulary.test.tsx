...
describe("useVocabulary", () => {
  it("does not reuse one year’s snapshot when the year changes", async () => {
    const getVocabulary = vi.spyOn(resourceApi, "getVocabulary").mockImplementation(
      async (schoolYearId): Promise<VocabularyResponse> => ({
    ...
    const { result, rerender } = renderHook(({ schoolYearId }) => useVocabulary(schoolYearId), {
      initialProps: { schoolYearId: "year-a" },
      wrapper,
    });
