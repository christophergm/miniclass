...
describe("request assembly", () => {
  ...
  it("attaches the bearer token to a roster write and serialises the body", async () => {
    ...
    await unwrap(
      client.POST("/api/school-years/{schoolYearID}/students", {
  ...
  it("serialises query parameters declared by the contract", async () => {
    const { client } = stubClient((request) => {
      expect(request.url).toBe(
        "https://api.test/api/school-years/year-1/guardian-relationships?student_id=student-1",
      );
      return jsonResponse([]);
    });

    await unwrapList(
      client.GET("/api/school-years/{schoolYearID}/guardian-relationships", {
  ...
  it("escapes an identifier in a path segment", async () => {
    ...
    await unwrapList(
      client.GET("/api/school-years/{schoolYearID}/students", {
        params: { path: { schoolYearID: "year/1" } },
      }),
    );
  });
...
describe("response handling", () => {
  ...
  it("accepts a 204 with no body", async () => {
    ...
    await expect(
      unwrapNoContent(
        client.DELETE("/api/school-years/{schoolYearID}/students/{studentID}", {
  ...
  it("turns an RFC 9457 problem into an ApiError carrying the raw field errors", async () => {
    ...
    await expect(
      unwrap(client.POST("/api/school-years", { body: { label: "" } })),
