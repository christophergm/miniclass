...
describe("inspectDevToken", () => {
  ...
  it("reports an unreadable token instead of throwing on anything undecodable", () => {
    ...
    expect(inspectDevToken("header.$$$not-base64$$$.signature", now)).toEqual({
      kind: "unreadable",
    });
    expect(
      inspectDevToken(`header.${base64url("a string, not an object")}.signature`, now),
  ...
  it("reports an expired token with its expiry", () => {
    ...
    expect(inspectDevToken(token({ exp: expiresAt.getTime() / 1000 }), now)).toEqual({
      kind: "expired",
      expiresAt,
    });
  });
  ...
  it("treats a token expiring exactly now as expired", () => {
    expect(inspectDevToken(token({ exp: now.getTime() / 1000 }), now)).toEqual({
      kind: "expired",
      expiresAt: now,
    });
  });
  ...
  it("reports a valid token with its expiry", () => {
    ...
    expect(
      inspectDevToken(token({ exp: expiresAt.getTime() / 1000, email: "dev@example.test" }), now),
    ).toEqual({ kind: "valid", expiresAt });
  });
});
