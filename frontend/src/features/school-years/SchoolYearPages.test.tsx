...
beforeEach(() => {
  ...
  mockQuery(useCreateSchoolYear, {
    mutate: vi.fn(),
    isPending: false,
    isError: false,
    error: null,
  });
  mockQuery(useUpdateSchoolYear, {
    mutate: vi.fn(),
    isPending: false,
    isError: false,
    error: null,
...
describe("SchoolYearSettingsPage", () => {
  it("opens the edit modal for a closed year and submits an owner reason when reopening", async () => {
    ...
    fireEvent.change(await screen.findByLabelText("Reason for reopening"), {
      target: { value: "Corrected an import error" },
    });
    fireEvent.click(await screen.findByRole("button", { name: "Reopen year" }));

    expect(mutate).toHaveBeenCalledWith(
      { state: "active", reason: "Corrected an import error" },
      expect.anything(),
    );
  });
  ...
  it("renders the not-found page instead of the workspace for a foreign year", () => {
    mockQuery(useSchoolYear, {
      ...
      isLoading: false,
      isError: true,
      error: new ApiError("http", "school year not found", 404, "resource-not-found"),
    });
    renderWorkspace();
...
describe("SchoolYearListPage", () => {
  it("starts creation from a modal rather than a persistent form", () => {
    ...
    fireEvent.change(screen.getByLabelText("Display label"), { target: { value: "2026–27" } });
    fireEvent.click(
      within(screen.getByRole("dialog", { name: "Create school year" })).getByRole("button", {
        name: "Create year",
      }),
    );
  ...
  it("links each year to its own workspace URL", () => {
    mockQuery(useSchoolYears, {
      data: [year({ id: "year-1", label: "2026–27", state: "active" })],
      isLoading: false,
      isError: false,
      error: null,
    });
