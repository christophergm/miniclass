...
vi.mock('@/lib/apiResources', async (importOriginal) => {
  ...
  return { ...actual, resourceApi: { ...actual.resourceApi, getMe: vi.fn(), updateHomeroomLabel: vi.fn() } }
})
...
const vocabulary: VocabularyResponse = { school_year_id: year.id, homeroom_label: 'form', grade_levels: [], homerooms: [] }

function renderSettings() {
  const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } })
  return render(<QueryClientProvider client={queryClient}><MemoryRouter initialEntries={['/settings']}><Routes><Route path="/settings" element={<SettingsPage />} /></Routes></MemoryRouter></QueryClientProvider>)
}
...
describe('SettingsPage', () => {
  it('is reachable without a school-year route and shows organization settings for an Owner', async () => {
    ...
    vi.mocked(useSchoolYears).mockReturnValue({ data: [year], isLoading: false, isError: false, error: null } as ReturnType<typeof useSchoolYears>)
  ...
  it('updates the organization homeroom label from the top-level settings route', async () => {
    ...
    vi.mocked(useSchoolYears).mockReturnValue({ data: [year], isLoading: false, isError: false, error: null } as ReturnType<typeof useSchoolYears>)
    vi.mocked(useVocabulary).mockReturnValue({ data: vocabulary, isLoading: false, isError: false, error: null } as ReturnType<typeof useVocabulary>)
    vi.mocked(useAdministrators).mockReturnValue({ data: { members: [] }, isLoading: false, error: null } as unknown as ReturnType<typeof useAdministrators>)
    vi.mocked(resourceApi.updateHomeroomLabel).mockResolvedValue({ organization_id: 'org-test', homeroom_label: 'class' })
