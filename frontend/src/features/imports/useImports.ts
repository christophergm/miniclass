...
export function usePreviewImport() {
  return useMutation<ImportPreview, unknown, ImportVariables>({
    mutationFn: ({ kind, schoolYearId, document }) =>
      resourceApi.previewImport(kind, schoolYearId, document),
  });
}
...
export function useCommitImport() {
  ...
  return useMutation<ImportPreview, unknown, ImportVariables & { contentHash: string }>({
    mutationFn: ({ kind, schoolYearId, document, contentHash }) =>
      resourceApi.commitImport(kind, schoolYearId, document, contentHash),
