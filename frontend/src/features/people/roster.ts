...
export const guardianApi = {
  ...
  listForYear: (schoolYearID: string) =>
    unwrapList(api.GET('/api/school-years/{schoolYearID}/guardian-relationships', { params: { path: { schoolYearID } } })),
 ...
 */
export function relatedPeopleByPerson(relationships: GuardianRelationship[], kind: PersonKind, counterparts: RelatedPerson[]): Map<string, RelatedPerson[]> {
  ...
  for (const relationship of relationships) {
    ...
    const counterpart = byId.get(counterpartId) ?? { id: counterpartId, display_name: counterpartId }
    index.set(personId, [...(index.get(personId) ?? []), counterpart])
  }
  for (const related of index.values()) {
    related.sort((left, right) => left.display_name.localeCompare(right.display_name, undefined, { numeric: true, sensitivity: 'base' }))
  }
  return index
}
