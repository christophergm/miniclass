...
const relationshipOptions: GuardianRelationshipType[] = [
  "parent",
  "guardian",
  "grandparent",
  "other",
...
export function GuardianRelationships({
  kind,
  schoolYearId,
  personId,
}: {
  ...
  const update = useRosterMutation(
    schoolYearId,
    (value: { relationshipId: string; type: GuardianRelationshipType }) =>
      guardianApi.update(schoolYearId, value.relationshipId, value.type),
  );
  const remove = useRosterMutation(schoolYearId, (relationshipId: string) =>
    guardianApi.remove(schoolYearId, relationshipId),
  );
  const create = useRosterMutation(
    ...
    }) => guardianApi.create(schoolYearId, value),
  );
  ...
  const otherPersonId = (relationship: GuardianRelationship) =>
    kind === "student" ? relationship.adult_id : relationship.student_id;
  ...
  return (
    <section
      ...
      aria-labelledby={`${kind}-guardian-heading`}
    >
      <h2 id={`${kind}-guardian-heading`} className="text-xl font-semibold">
      ...
      {error !== null && (
        <p
          ...
          role="alert"
        >
          {error instanceof Error ? error.message : "Unable to update guardian relationships."}
        </p>
      )}
      {isLoading ? (
        <p className="mt-6 text-sm text-muted-foreground" role="status">
          Loading guardian relationships…
        </p>
      ) : (
        <>
          ...
          {relationships.length === 0 &&
            (kind === "student" ? (
              <p
                ...
                role="status"
              >
                This student has no guardian yet. Nobody can be reached about this child. This is a
                warning only; you can still save the roster record.
              </p>
            ) : (
              <p className="mt-6 text-sm text-muted-foreground">
                No guardian relationships recorded.
              </p>
            ))}
          <ul className="mt-4 space-y-3">
            {relationships.map((relationship) => (
              <RelationshipRow
                ...
                label={displayNames.get(otherPersonId(relationship)) ?? otherPersonId(relationship)}
                disabled={isSaving}
          ...
          <div className="mt-6 flex flex-wrap items-end gap-3 border-t pt-6">
            <label className="min-w-64 flex-1 text-sm font-medium">
              {kind === "student" ? "Adult" : "Student"}
              <select
                className="mt-2 flex h-9 w-full rounded-md border bg-transparent px-3 text-sm"
                value={selectedPersonId}
                onChange={(event) => setSelectedPersonId(event.target.value)}
              >
                <option value="">Choose a {kind === "student" ? "adult" : "student"}</option>
                {candidates
                  .filter(
                    (candidate) =>
                      !relationships.some(
                        (relationship) => otherPersonId(relationship) === candidate.id,
                      ),
                  )
                  .map((candidate) => (
            ...
            <label className="text-sm font-medium">
              ...
              <select
                ...
                className="mt-2 flex h-9 rounded-md border bg-transparent px-3 text-sm"
                value={relationshipType}
                onChange={(event) =>
                  setRelationshipType(event.target.value as GuardianRelationshipType)
                }
              >
                {relationshipOptions.map((option) => (
                  <option key={option} value={option}>
                    {labelForRelationship(option)}
                  </option>
                ))}
              </select>
            </label>
...
function RelationshipRow({
  ...
  label: string;
  disabled: boolean;
  onSave: (relationshipId: string, type: GuardianRelationshipType) => void;
  ...
  return (
    <li className="flex flex-wrap items-center justify-between gap-3 rounded-md border px-3 py-3 text-sm">
      <Link
        className="text-primary hover:underline"
        to={`/y/${schoolYearId}/${personPath}/${otherPersonId}`}
      >
        {label}
      </Link>
      <div className="flex items-center gap-2">
        <label className="sr-only" htmlFor={`relationship-${relationship.id}`}>
          Relationship for {label}
        </label>
        <select
          id={`relationship-${relationship.id}`}
          value={type}
          onChange={(event) => setType(event.target.value as GuardianRelationshipType)}
        ...
        <Button
          type="button"
          size="sm"
          disabled={disabled || type === relationship.relationship_type}
          onClick={() => onSave(relationship.id, type)}
        >
          Save
        </Button>
        <Button
