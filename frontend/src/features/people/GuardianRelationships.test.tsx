...
describe("guardian relationships", () => {
  it("edits a typed relationship from a student detail", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Relationship for Morgan Lee"), {
      target: { value: "guardian" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Save" }));
    await waitFor(() =>
      expect(update).toHaveBeenCalledWith("year-1", "relationship-1", "guardian"),
    );
  });

  it("loads the reverse relationship view from an adult detail", async () => {
    vi.spyOn(guardianApi, "listForAdult").mockResolvedValue([
      relationship({ relationship_type: "grandparent" }),
    ]);
  ...
  it("adds a relationship with the create endpoint and removes it by identifier", async () => {
    ...
    vi.spyOn(studentApi, "list").mockResolvedValue([
      riley,
      { ...riley, id: "student-2", display_name: "Sam Stone" },
    ]);
    const create = vi
      .spyOn(guardianApi, "create")
      .mockResolvedValue(
        relationship({ id: "relationship-2", student_id: "student-2", relationship_type: "other" }),
      );
  ...
  it("reads each list once per mount, including under StrictMode", async () => {
    const listForStudent = vi
      .spyOn(guardianApi, "listForStudent")
