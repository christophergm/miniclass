...
export function PeopleListPage({ kind }: PageProps) {
  ...
  const restore = useRosterMutation<{ id: string; reason: string }, Student | Adult>(
    schoolYearId,
    async ({ id, reason }): Promise<Student | Adult> =>
      kind === "student"
        ? studentApi.restore(schoolYearId!, id, reason)
        : adultApi.restore(schoolYearId!, id, reason),
  );
  ...
  const missingGradeCount =
    kind === "student"
      ? (peopleQuery.data ?? []).filter((person) => (person as Student).grade_level_id == null)
          .length
      : 0;
  ...
  return (
    <PageFrame>
            ...
            <>
              ...
              <label className="text-sm font-medium" htmlFor="student-homeroom">
                ...
                <select
                  ...
                  className="mt-2 flex h-9 w-full rounded-md border bg-transparent px-3 text-sm"
                  value={homeroomId}
                  onChange={(event) => setHomeroomId(event.target.value)}
                >
                  <option value="">
                    All {(vocabulary?.homeroom_label ?? "homeroom").toLowerCase()}s
                  </option>
      ...
      {missingGradeCount > 0 && (
        <p
          className="mt-4 rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-900"
          role="status"
        >
          {missingGradeCount} student{missingGradeCount === 1 ? "" : "s"} have no grade yet. This is
          a warning; you can still save roster changes.
        </p>
      )}

      {isLoading && (
        <p className="mt-8 text-sm text-muted-foreground" role="status">
          Loading {copy.plural.toLowerCase()}…
        </p>
      )}
      {error !== null && (
        <p
    ...
    <Table className="mt-8" aria-label={copy.plural}>
      <TableHeader>
        <TableRow>
          ...
          {kind === "student" ? (
            <>
              <SortableHead label="Grade" sortKey="grade" sort={sort} onSort={onSort} />
              <SortableHead label="Homeroom" sortKey="homeroom" sort={sort} onSort={onSort} />
            </>
          ) : (
            <>
      ...
      <TableBody>
        {people.map((person) => (
          <TableRow
            key={person.id}
            className={person.deleted_at ? "bg-muted/50 text-muted-foreground" : undefined}
          >
            <TableCell>
              <Link
                className="font-medium text-primary hover:underline"
                to={`/y/${schoolYearId}/${copy.path}/${person.id}`}
              >
                {person.display_name}
              </Link>
            </TableCell>
            {kind === "student" ? (
              <>
                <TableCell>
                  {gradeLabels.get((person as Student).grade_level_id ?? "") ??
                    ((person as Student).grade_level_id == null
                      ? "Missing grade"
                      : (person as Student).grade_level_id)}
                </TableCell>
                <TableCell>
                  {homeroomLabels.get((person as Student).homeroom_id) ??
                    (person as Student).homeroom_id}
                </TableCell>
              </>
            ) : (
              <>
                ...
                <TableCell className="capitalize">
                  {(person as Adult).participation_intent ?? "Not declared"}
                </TableCell>
              </>
            )}
            <TableCell>
    ...
    return (
      <PageFrame>
        <p className="text-sm text-muted-foreground" role="status">
          Loading {copy.singular}…
        </p>
      </PageFrame>
    );
  if (error && !isNew && !person)
    return (
      <PageFrame>
        <p
          ...
          role="alert"
        >
          {errorMessage(error, `Unable to load ${copy.singular}.`)}
        </p>
      </PageFrame>
    );
    ...
    <PageFrame>
      ...
      {error !== null && (
        <p
          ...
          role="alert"
        >
          {errorMessage(error, `Unable to save this ${copy.singular}.`)}
        </p>
      )}
      <form
        className="mt-8 max-w-2xl space-y-6 rounded-lg border bg-card p-6"
      ...
      >
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <Field
            ...
            onChange={(value) => setValues({ ...values, legal_family_name: value })}
          />
          <Field
            label="Preferred given name"
            name="preferred_given_name"
            value={values.preferred_given_name}
            error={fieldErrors.preferred_given_name}
          ...
          {kind === "student" ? (
            <>
              <Select
                label="Grade"
                name="grade_level_id"
                value={(values as StudentInputValues).grade_level_id}
                error={fieldErrors.grade_level_id}
                onChange={(value) =>
                  setValues({ ...values, grade_level_id: value } as StudentInputValues)
                }
              ...
              <Select
                ...
                name="homeroom_id"
                value={(values as StudentInputValues).homeroom_id}
                error={fieldErrors.homeroom_id}
                onChange={(value) =>
                  setValues({ ...values, homeroom_id: value } as StudentInputValues)
                }
                options={homerooms.map((homeroom) => ({
                  value: homeroom.id,
                  label: homeroom.name,
                }))}
            ...
            <>
              <Field
                ...
                name="email"
                type="email"
                value={(values as AdultInputValues).email}
                error={fieldErrors.email}
                onChange={(value) => setValues({ ...values, email: value } as AdultInputValues)}
              />
              <Field
                label="Phone"
                name="phone"
                value={(values as AdultInputValues).phone}
                error={fieldErrors.phone}
                onChange={(value) => setValues({ ...values, phone: value } as AdultInputValues)}
              />
              <label className="text-sm font-medium" htmlFor="participation_intent">
                Participation intent
                <span className="mt-2 block text-xs font-normal text-muted-foreground">
                  Used for adult planning, not a person role.
                </span>
                <select
                  id="participation_intent"
                  className="mt-2 flex h-9 w-full rounded-md border bg-transparent px-3 text-sm"
                  value={(values as AdultInputValues).participation_intent}
                  onChange={(event) =>
                    setValues({
                      ...values,
                      participation_intent: event.target
                        .value as AdultInputValues["participation_intent"],
                    } as AdultInputValues)
                  }
                >
                ...
                {fieldErrors.participation_intent && (
                  <FieldError message={fieldErrors.participation_intent} />
                )}
              </label>
            </>
          )}
        ...
        <div className="flex gap-3">
          ...
          <Button asChild type="button" variant="outline">
            <Link to={`/y/${schoolYearId}/${copy.path}`}>Cancel</Link>
          </Button>
        </div>
      </form>
      {!isNew && person && (
...
function valuesFromPerson(kind: PersonKind, person: PersonSummary): PersonInputValues {
  ...
  return kind === "student"
    ...
    : {
        ...common,
        email: (person as Adult).email ?? "",
        phone: (person as Adult).phone ?? "",
        participation_intent: (person as Adult).participation_intent ?? "",
      };
}
...
function savePerson(
  ...
  return personId
    ? adultApi.update(schoolYearId, personId, body)
    : adultApi.create(schoolYearId, body);
}
...
function filterPeople(
  ...
  return people.filter((person) => {
    ...
    const matchesSearch =
      ...
      person.legal_given_name.toLocaleLowerCase().includes(normalized) ||
      person.legal_family_name.toLocaleLowerCase().includes(normalized);
    return (
      matchesSearch &&
      (kind !== "student" ||
        ((!gradeLevelId || student.grade_level_id === gradeLevelId) &&
          (!homeroomId || student.homeroom_id === homeroomId)))
    );
  });
}
...
// unavailable.
const participationRank: Record<NonNullable<ParticipationIntent>, number> = {
  lead: 0,
  help: 1,
  unavailable: 2,
};
...
function compareBySortKey(
  ...
  left: PersonSummary,
  right: PersonSummary,
  gradeOrdinals: Map<string, number>,
  homeroomNames: Map<string, string>,
) {
...
function compareByName(left: PersonSummary, right: PersonSummary) {
  return (
    compareValues(left.legal_family_name, right.legal_family_name) ||
    compareValues(left.legal_given_name, right.legal_given_name)
  );
}
...
function Field({
  ...
  return (
    <label className="text-sm font-medium" htmlFor={name}>
      ...
      />
      {hint && !error && (
        <span id={`${name}-hint`} className="mt-1 block text-xs font-normal text-muted-foreground">
          {hint}
        </span>
      )}
...
function FieldError({ id, message }: { id?: string; message: string }) {
  return (
    <span id={id} className="mt-1 block text-xs font-normal text-destructive" role="alert">
      {message}
    </span>
  );
}
    ...
    <span className="flex flex-wrap gap-x-2 gap-y-1">
      {related.map((person) => (
        <Link
          ...
          to={`/y/${schoolYearId}/${counterpartPath}/${person.id}`}
        >
          {person.display_name}
        </Link>
      ))}
...
function MissingSchoolYear({ kind }: PageProps) {
  return (
    <>
      ...
      <h1 className="mt-2 text-3xl font-semibold tracking-tight">Choose a school year</h1>
      <p className="mt-3 max-w-xl text-sm text-muted-foreground">
        {pageCopy[kind].plural} belong to a school year. Open a year workspace before managing this
        roster.
      </p>
