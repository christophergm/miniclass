...
const students: Student[] = [
  {
    ...
    preferred_given_name: "Addie",
    display_name: "Addie Zephyr",
  ...
  },
  {
    ...
    ...timestamps,
    id: "student-1",
    legal_given_name: "Bea",
    legal_family_name: "Apple",
    display_name: "Bea Apple",
  ...
  },
  {
    ...
    ...timestamps,
    id: "student-3",
    legal_given_name: "Ari",
    legal_family_name: "Apple",
    preferred_given_name: "Aria",
    display_name: "Aria Apple",
    grade_level_id: "grade-2",
    homeroom_id: "homeroom-b",
  },
];
const deletedStudent: Student = {
  ...
  legal_family_name: "Vale",
  display_name: "Sam Vale",
  grade_level_id: "grade-1",
  homeroom_id: "homeroom-a",
...
const adults: Adult[] = [
  ...
  },
  {
    ...
    id: "adult-2",
    legal_given_name: "Jo",
    legal_family_name: "Kim",
    display_name: "Jo Kim",
    participation_intent: "lead",
  },
];
...
function stubGuardianRelationships(relationships: GuardianRelationship[] = []) {
  ...
  vi.spyOn(guardianApi, "listForStudent").mockImplementation(async (_schoolYearID, student_id) =>
    relationships.filter((edge) => edge.student_id === student_id),
  );
  vi.spyOn(guardianApi, "listForAdult").mockImplementation(async (_schoolYearID, adult_id) =>
    relationships.filter((edge) => edge.adult_id === adult_id),
  );
}
...
describe("people roster pages", () => {
  it("lists every guardian of a student, linked to the adult record", async () => {
    ...
    stubGuardianRelationships([
      relationship("rel-1", "adult-1", "student-2"),
      relationship("rel-2", "adult-2", "student-2"),
    ]);
    ...
    expect(await within(table).findByRole("link", { name: "Mo Lee" })).toHaveAttribute(
      "href",
      "/y/year-1/adults/adult-1",
    );
    expect(within(table).getByRole("link", { name: "Jo Kim" })).toHaveAttribute(
      "href",
      "/y/year-1/adults/adult-2",
    );
  });
  ...
  it("lists the children an adult is a guardian of, linked to the student record", async () => {
    ...
    stubGuardianRelationships([
      relationship("rel-1", "adult-1", "student-2"),
      relationship("rel-2", "adult-1", "student-1"),
    ]);
    ...
    expect(await within(table).findByRole("link", { name: "Addie Zephyr" })).toHaveAttribute(
      "href",
      "/y/year-1/students/student-2",
    );
    expect(within(table).getByRole("link", { name: "Bea Apple" })).toHaveAttribute(
      "href",
  ...
  it("warns without blocking on the detail page of a student with no guardian", async () => {
    ...
    expect(
      await screen.findByText(
        "This student has no guardian yet. Nobody can be reached about this child. This is a warning only; you can still save the roster record.",
      ),
    ).toBeInTheDocument();
  ...
  it("keeps the external identifier out of the roster tables but on the detail page", async () => {
    ...
    vi.spyOn(adultApi, "list").mockResolvedValue([
      { ...adult, external_identifier: "platform-4c1" },
    ]);
    ...
    const studentTable = await screen.findByRole("table", { name: "Students" });
    expect(
      within(studentTable).queryByRole("columnheader", { name: "External ID" }),
    ...
    const adultTable = await screen.findByRole("table", { name: "Adults" });
    expect(
      within(adultTable).queryByRole("columnheader", { name: "External ID" }),
    ).not.toBeInTheDocument();
  ...
  it("sorts students by grade ordinal rather than label, and puts a missing grade last", async () => {
    ...
    vi.spyOn(studentApi, "list").mockResolvedValue([
      {
        ...students[1],
        id: "student-second",
        display_name: "Sec Ond",
        legal_family_name: "Ond",
        grade_level_id: "grade-2",
      },
      {
        ...students[1],
        id: "student-none",
        display_name: "No Grade",
        legal_family_name: "Grade",
        grade_level_id: null,
      },
      {
        ...students[1],
        id: "student-k",
        display_name: "Kin Der",
        legal_family_name: "Der",
        grade_level_id: "grade-k",
      },
      {
        ...students[1],
        id: "student-first",
        display_name: "Fir St",
        legal_family_name: "St",
        grade_level_id: "grade-1",
      },
    ]);
    ...
    const table = await screen.findByRole("table", { name: "Students" });
    await waitFor(() =>
      expect(within(table).getByRole("button", { name: "Grade" })).toBeInTheDocument(),
    );

    fireEvent.click(within(table).getByRole("button", { name: "Grade" }));
    expect(rowNames(table)).toEqual(["Kin Der", "Fir St", "Sec Ond", "No Grade"]);
    expect(within(table).getByRole("columnheader", { name: "Grade" })).toHaveAttribute(
      "aria-sort",
      "ascending",
    );
  ...
  it("sorts adults by the spec order of participation intent, then by email with blanks last", async () => {
    vi.spyOn(adultApi, "list").mockResolvedValue([
      {
        ...adult,
        id: "adult-unavailable",
        display_name: "Una Vail",
        legal_family_name: "Vail",
        email: "zoe@example.test",
        participation_intent: "unavailable",
      },
      {
        ...adult,
        id: "adult-none",
        display_name: "Nora None",
        legal_family_name: "None",
        email: undefined,
        participation_intent: null,
      },
      {
        ...adult,
        id: "adult-help",
        display_name: "Hal Help",
        legal_family_name: "Help",
      ...
      },
      {
        ...adult,
        id: "adult-lead",
        display_name: "Lee Lead",
        legal_family_name: "Lead",
    ...
    const table = await screen.findByRole("table", { name: "Adults" });
    await waitFor(() =>
      expect(within(table).getByRole("button", { name: "Participation" })).toBeInTheDocument(),
    );
    ...
    expect(within(table).getByRole("columnheader", { name: "Participation" })).toHaveAttribute(
      "aria-sort",
      "none",
    );
  });
  ...
  it("asks for deleted students only when the filter is set, and restores one with a reason", async () => {
    const list = vi
      .spyOn(studentApi, "list")
      .mockImplementation(async (_schoolYearId, includeDeleted) =>
        includeDeleted ? [students[1], deletedStudent] : [students[1]],
      );
    const restore = vi
      .spyOn(studentApi, "restore")
      .mockResolvedValue({ ...deletedStudent, deleted_at: undefined });
    ...
    expect(
      within(screen.getByText("Bea Apple").closest("tr")!).queryByRole("button", {
        name: "Restore",
      }),
    ).not.toBeInTheDocument();
    ...
    fireEvent.click(within(deletedRow).getByRole("button", { name: "Restore" }));
    await waitFor(() =>
      expect(restore).toHaveBeenCalledWith("year-1", "student-4", "deleted by mistake"),
    );
  });
  ...
  it.each([
    ...
    "reads the year’s guardian edges in a bounded number of requests on the %s",
    async (_surface, path, linkName, vocabularyCalls, yearWideCalls, perStudentCalls, perAdultCalls) => {
      const manyStudents: Student[] = [
        students[0],
        ...Array.from({ length: 180 }, (_value, index) => ({
          ...ids,
          ...timestamps,
          id: `student-extra-${index}`,
          legal_given_name: "Sam",
          legal_family_name: `Extra${index}`,
          display_name: `Sam Extra${index}`,
          grade_level_id: "grade-1",
          homeroom_id: "homeroom-a",
        })),
      ];
