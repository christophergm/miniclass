...
export function ProgramMembershipPage() {
  ...
  const submitSession = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); createSession.mutate({ name: sessionName, meeting_dates: sessionDates.split(',').map((date) => date.trim()).filter(Boolean) }, { onSuccess: () => { setSessionName(''); setSessionDates('') } }) }
  return <PageFrame>
    ...
    <Card title="Sessions" description="Sessions are ordered by first meeting date, then name. Enter every meeting date as an ISO date separated by commas.">
      <form className="mt-4 grid gap-3 sm:grid-cols-[1fr_2fr_auto]" onSubmit={submitSession}><Input aria-label="Session name" disabled={readOnly} onChange={(event) => setSessionName(event.target.value)} placeholder="Session name" value={sessionName} /><Input aria-label="Meeting dates" disabled={readOnly} onChange={(event) => setSessionDates(event.target.value)} placeholder="2026-10-02, 2026-10-09" value={sessionDates} /><Button disabled={readOnly || createSession.isPending || !sessionName.trim() || !sessionDates.trim()} type="submit">Create session</Button></form>{createSession.isError && <Problem error={createSession.error} fallback="Unable to create the session." />}
      <div className="mt-5 grid gap-3 sm:grid-cols-2">{(sessions.data ?? []).map((session) => <Link className="rounded-md border p-4 hover:border-primary/50" key={session.id} to={`/y/${schoolYearId}/programs/${programId}/sessions/${session.id}`}><div className="flex items-center justify-between gap-3"><h3 className="font-medium">{session.name}</h3><span className="rounded-full bg-secondary px-2 py-1 text-xs">{stateLabel(session.state)}</span></div><p className="mt-2 text-sm text-muted-foreground">{session.meeting_dates?.length ?? 0} meeting dates · {(session.feasibility_warnings ?? []).length} warning{session.feasibility_warnings?.length === 1 ? '' : 's'}</p></Link>)}</div>
    </Card>
  </PageFrame>
}
...
export function SessionPage() {
  ...
  return <PageFrame>
    ...
    <Card title="Session details" description="Update the name and, optionally, replace all meeting dates atomically. Enter one or more ISO dates separated by commas."><form className="mt-4 flex flex-wrap gap-3" onSubmit={(event) => { event.preventDefault(); updateSession.mutate({ sessionID: sessionId, value: { ...(sessionName ? { name: sessionName } : {}), ...(sessionDates.trim() ? { meeting_dates: sessionDates.split(',').map((date) => date.trim()).filter(Boolean) } : {}) } }) }}><Input aria-label="Edit session name" disabled={readOnly} onChange={(event) => setSessionName(event.target.value)} placeholder={current.name} value={sessionName} /><Input aria-label="Edit meeting dates" disabled={readOnly} onChange={(event) => setSessionDates(event.target.value)} placeholder={current.meeting_dates?.join(', ')} value={sessionDates} /><Button disabled={readOnly || updateSession.isPending || (!sessionName && !sessionDates.trim())} type="submit">Save details</Button></form>{updateSession.isError && <Problem error={updateSession.error} fallback="Unable to update session details." />}</Card>
