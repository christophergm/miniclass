...
import type { CatalogFeasibilityWarning, InterestArea, ObjectiveWeightOverrides, ObjectiveWeights, SchoolYear, Session, SessionNonParticipation } from '@/lib/apiResources'
...
import {
  useAddProgramMembership, useCatalogFeasibility, useCreateInterestArea,
  ...
  useUpdateInterestArea, useUpdateProgramObjectiveWeights,
...
function MeetingDateDraftList({ dates, onChange }: { dates: string[]; onChange: (dates: string[]) => void }) {
  ...
  return <div className="space-y-3">{dates.length === 0 && <p className="text-sm text-muted-foreground" role="status">Add at least one meeting date.</p>}{dates.some((date) => !date) && <p className="text-sm text-destructive" role="alert">Each meeting date is required.</p>}<div className="space-y-2">{dates.map((date, index) => <div className="flex items-center gap-2" key={`${date}-${index}`}><Input aria-label={`Meeting date ${index + 1}`} onChange={(event) => onChange(dates.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} required type="date" value={date} /><Button aria-label={`Remove meeting date ${index + 1}`} disabled={dates.length === 1} onClick={() => onChange(dates.filter((_, itemIndex) => itemIndex !== index))} size="sm" type="button" variant="outline">Remove</Button></div>)}</div><div className="flex gap-2"><Input aria-label="New meeting date" onChange={(event) => setNewDate(event.target.value)} type="date" value={newDate} /><Button disabled={!newDate} onClick={addDate} type="button" variant="outline">Add date</Button></div></div>
}
...
export function ProgramListPage() {
  ...
  return <PageFrame>
    ...
    <div className="mt-2 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-semibold tracking-tight">Programs</h1><p className="mt-2 text-sm text-muted-foreground">Author membership, interest areas, sessions, and the catalog for this school year.</p></div><div className="flex gap-2"><Button asChild variant="outline"><Link to={`/y/${schoolYearId}/settings`}>Settings</Link></Button></div></div>
    {readOnly && <ReadOnlyNotice />}
    {(missingGrades.data?.missing_grade_count ?? 0) > 0 && <p className="mt-6 rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-900">{missingGrades.data?.missing_grade_count} student{missingGrades.data?.missing_grade_count === 1 ? '' : 's'} are excluded from membership until their grade is known. <Link className="font-medium underline" to={`/y/${schoolYearId}/students`}>Fix this in the roster</Link>.</p>}
    <ModalForm dirty={Boolean(name.trim())} onClose={() => setCreateOpen(false)} open={createOpen} title="Create program" description="Give this programme a name to begin authoring."><form className="space-y-4" onSubmit={(event) => { event.preventDefault(); create.mutate(name.trim(), { onSuccess: () => { setName(''); setCreateOpen(false) } }) }}><label className="block text-sm font-medium">Program name<Input aria-label="Program name" className="mt-2" onChange={(event) => setName(event.target.value)} placeholder="Program name" required value={name} /></label><div className="flex gap-2"><Button disabled={create.isPending || !name.trim()} type="submit">{create.isPending ? 'Creating…' : 'Create program'}</Button><Button onClick={() => setCreateOpen(false)} type="button" variant="outline">Cancel</Button></div>{create.isError && <Problem error={create.error} fallback="Unable to create the program." />}</form></ModalForm>
    ...
    {!programs.isLoading && !programs.isError && <div className="mt-8 divide-y rounded-lg border bg-card shadow-sm">{(programs.data ?? []).map((program) => <Link className="flex items-center justify-between gap-4 p-5 first:rounded-t-lg last:rounded-b-lg hover:bg-accent/50" key={program.id} to={`/y/${schoolYearId}/programs/${program.id}`}><div><h2 className="font-semibold">{program.name}</h2><p className="mt-2 text-sm text-muted-foreground">Membership · interest areas · sessions</p></div><span aria-hidden="true" className="text-muted-foreground">→</span></Link>)}</div>}

    <div className="mt-8 flex items-center gap-4">
      <Button disabled={readOnly} onClick={() => { setName(''); setCreateOpen(true) }} type="button">Create program</Button>
    </div>
...
function ProgramBreadcrumb({ schoolYearId, programId, programName, current }: { schoolYearId: string; programId: string; programName: string; current: string }) {
  return <Breadcrumb aria-label="Program breadcrumb"><BreadcrumbList><BreadcrumbItem><BreadcrumbLink asChild><Link to={`/y/${schoolYearId}/programs`}>Programs</Link></BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem>{current ? <BreadcrumbLink asChild><Link to={`/y/${schoolYearId}/programs/${programId}`}>{programName}</Link></BreadcrumbLink> : <BreadcrumbPage>{programName}</BreadcrumbPage>}</BreadcrumbItem>{current ? <><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbPage>{current}</BreadcrumbPage></BreadcrumbItem></> : null}</BreadcrumbList></Breadcrumb>
}

function ProgramSettingsBreadcrumb({ schoolYearId, programId, programName, current }: { schoolYearId: string; programId: string; programName: string; current: string }) {
  return <Breadcrumb aria-label="Program breadcrumb"><BreadcrumbList><BreadcrumbItem><BreadcrumbLink asChild><Link to={`/y/${schoolYearId}/programs`}>Programs</Link></BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbLink asChild><Link to={`/y/${schoolYearId}/programs/${programId}`}>{programName}</Link></BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbLink asChild><Link to={`/y/${schoolYearId}/programs/${programId}/settings`}>Settings</Link></BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbPage>{current}</BreadcrumbPage></BreadcrumbItem></BreadcrumbList></Breadcrumb>
}
...
export function ProgramDetailPage() {
  ...
  return <PageFrame>
    <ProgramBreadcrumb current="" programId={programId} programName={selected?.name ?? 'Program'} schoolYearId={schoolYearId} />
    ...
    <Card title="Sessions" description="Sessions are ordered by first meeting date, then name. Add each meeting date in the session form.">
      ...
      <Button className="mt-5" disabled={readOnly} onClick={() => { setSessionDraft({ name: '', meetingDates: [] }); setSessionEditor('create') }} type="button">Create session</Button>
...
export function ProgramMembershipPage() {
  ...
  if (!schoolYearId || !programId) return <PageFrame><p>Program is required.</p></PageFrame>
  return <PageFrame>
    <ProgramSettingsBreadcrumb current="Membership" programId={programId} programName={selected?.name ?? 'Program'} schoolYearId={schoolYearId} />
...
export function ProgramInterestAreasPage() {
  ...
  return <PageFrame>
    <ProgramSettingsBreadcrumb current="Interest areas" programId={programId} programName={selected?.name ?? 'Program'} schoolYearId={schoolYearId} />
...
export function SessionPage() {
  ...
  const [sessionDraft, setSessionDraft] = useState<SessionDraft>({ name: '', meetingDates: [] })
  const [nonParticipationEditor, setNonParticipationEditor] = useState<'create' | SessionNonParticipation | null>(null)
  const [nonParticipationDraft, setNonParticipationDraft] = useState({ studentId: '', reason: '' })
  const [nonParticipationBaseline, setNonParticipationBaseline] = useState({ studentId: '', reason: '' })
  ...
  const closeTransitionPreview = () => { setTransitionPreview(null); setTransitionReason('') }
  const openSessionEditor = () => { setSessionDraft({ name: current.name, meetingDates: [...(current.meeting_dates ?? [])] }); setSessionEditorOpen(true) }
  const submitSession = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); updateSession.mutate({ sessionID: sessionId, value: { name: sessionDraft.name.trim(), meeting_dates: sessionDraft.meetingDates } }, { onSuccess: () => setSessionEditorOpen(false) }) }
  const openCreateNonParticipation = () => { setNonParticipationDraft({ studentId: '', reason: '' }); setNonParticipationBaseline({ studentId: '', reason: '' }); setNonParticipationEditor('create') }
  ...
  return <PageFrame>
    ...
    <div className="mt-2 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-semibold tracking-tight">{current.name}</h1><p className="mt-2 text-sm text-muted-foreground">Meeting dates: {current.meeting_dates?.length ? current.meeting_dates.join(', ') : 'not set'}</p></div><div className="flex flex-wrap items-end justify-end gap-3"><label className="text-sm font-medium"><span className="sr-only">State</span><select aria-label="Session state" className="mt-1 block h-9 rounded-md border bg-transparent px-3 text-sm" disabled={readOnly} onChange={(event) => { const value = event.target.value; setTransitionState(value === current.state ? '' : value); setTransitionPreview(null) }} value={transitionState || current.state}>{availableStates.map((state) => <option key={state} value={state}>{stateLabel(state)}</option>)}</select></label><Button disabled={readOnly || !transitionState || transitionState === current.state || transition.isPending} onClick={() => performTransition(false)} type="button">{transitionNeedsConfirmation ? 'Preview transition...' : 'Transition'}</Button><Button disabled={readOnly} onClick={openSessionEditor} type="button">Edit session</Button><Link className="text-sm font-medium text-primary hover:underline" to={`/y/${schoolYearId}/programs/${programId}/sessions/${sessionId}/assignment-planner`}>Assignment planner</Link></div></div>
    ...
    {transition.isError && <Problem error={transition.error} fallback="Unable to change session state." />}
    <ModalForm dirty={false} onClose={closeTransitionPreview} open={transitionPreview !== null} title={`Preview transition to ${transitionPreview ? stateLabel(transitionPreview.state) : ''}`} description="Review the consequences and provide a reason before confirming this backward transition.">{transitionPreview && <div className="text-sm"><div className="rounded-md border border-amber-300 bg-amber-50 p-4 text-amber-950"><strong>Review before confirming {stateLabel(transitionPreview.state)}</strong>{transitionPreview.warnings.length === 0 ? <p className="mt-2">No additional warnings were reported.</p> : transitionPreview.warnings.map((warning) => <div className="mt-2" key={warning.message}><p>{warning.message}</p>{warning.invalidation_summary?.map((summary) => <p className="mt-1" key={summary}>• {summary}</p>)}</div>)}</div><label className="mt-4 block font-medium">Reason for this backward transition<Input aria-label="Transition reason" className="mt-1" disabled={readOnly} onChange={(event) => setTransitionReason(event.target.value)} value={transitionReason} /></label><div className="mt-3 flex gap-2"><Button disabled={!transitionReason.trim() || transition.isPending} onClick={() => performTransition(true)} type="button">Confirm transition</Button><Button onClick={closeTransitionPreview} type="button" variant="outline">Cancel</Button></div></div>}</ModalForm>
    <OfferingSummary readOnly={readOnly} grades={gradeLevels} schoolYearId={schoolYearId} programId={programId} sessionId={sessionId} />
    <Card title="Session non-participation" description="This does not remove annual programme membership. Each exclusion requires an auditable reason."><Button disabled={readOnly || (memberships.data ?? []).filter((member) => !excludedIds.has(member.student_id)).length === 0} onClick={openCreateNonParticipation} type="button">Mark not participating</Button><Table className="mt-6" aria-label="Session non-participation"><TableHeader><TableRow><TableHead>Student</TableHead><TableHead>Reason</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader><TableBody>{(exclusions.data ?? []).map((item) => <NonParticipationRow item={item} key={item.id} readOnly={readOnly} onEdit={() => openEditNonParticipation(item)} remove={deleteExclusion} studentName={(memberships.data ?? []).find((member) => member.student_id === item.student_id)?.legal_given_name ?? item.student_id} />)}</TableBody></Table>{createExclusion.isError && <Problem error={createExclusion.error} fallback="Unable to record non-participation." />}</Card>
    <ModalForm dirty={sessionEditorOpen && (sessionDraft.name !== current.name || JSON.stringify(sessionDraft.meetingDates) !== JSON.stringify(current.meeting_dates ?? []))} onClose={() => setSessionEditorOpen(false)} open={sessionEditorOpen} title="Edit session" description="The name and meeting dates save together. Keep at least one date."><SessionForm error={updateSession.error} onChange={setSessionDraft} onSubmit={submitSession} pending={updateSession.isPending} submitLabel="Save session" value={sessionDraft} /></ModalForm>
    <ModalForm dirty={nonParticipationDraft.studentId !== nonParticipationBaseline.studentId || nonParticipationDraft.reason !== nonParticipationBaseline.reason} onClose={() => setNonParticipationEditor(null)} open={nonParticipationEditor !== null} title={nonParticipationEditor === 'create' ? 'Mark not participating' : 'Edit non-participation'} description="A reason is required and will be recorded in the audit history."><form className="space-y-4" onSubmit={submitNonParticipation}>{nonParticipationEditor === 'create' && <label className="block text-sm font-medium">Student<select aria-label="Non-participating student" className="mt-2 flex h-9 w-full rounded-md border bg-transparent px-3 text-sm" onChange={(event) => setNonParticipationDraft({ ...nonParticipationDraft, studentId: event.target.value })} required value={nonParticipationDraft.studentId}><option value="">Choose a programme member</option>{(memberships.data ?? []).filter((member) => !excludedIds.has(member.student_id)).map((member) => <option key={member.student_id} value={member.student_id}>{member.legal_given_name} {member.legal_family_name}</option>)}</select></label>}<label className="block text-sm font-medium">Reason<Input aria-label="Non-participation reason" className="mt-2" onChange={(event) => setNonParticipationDraft({ ...nonParticipationDraft, reason: event.target.value })} placeholder="Reason (required)" required value={nonParticipationDraft.reason} /></label><div className="flex gap-2"><Button disabled={createExclusion.isPending || updateExclusion.isPending || (nonParticipationEditor === 'create' && !nonParticipationDraft.studentId) || !nonParticipationDraft.reason.trim()} type="submit">{nonParticipationEditor === 'create' ? 'Mark not participating' : 'Save reason'}</Button><Button onClick={() => setNonParticipationEditor(null)} type="button" variant="outline">Cancel</Button></div>{(createExclusion.isError || updateExclusion.isError) && <Problem error={createExclusion.error || updateExclusion.error} fallback="Unable to save non-participation." />}</form></ModalForm>
  </PageFrame>
}

function Warnings({ warnings }: { warnings: CatalogFeasibilityWarning[] }) { if (warnings.length === 0) return null; return <section className="mt-4 rounded-md border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950" role="status"><strong>Catalog feasibility warning</strong><ul className="mt-2 space-y-2">{warnings.map((warning) => <li key={warning.id}><p>{warning.message}</p><p className="mt-1">Advisory only — you can continue authoring.</p></li>)}</ul></section> }

function NonParticipationRow({ item, readOnly, onEdit, remove, studentName }: { item: SessionNonParticipation; readOnly: boolean; onEdit: () => void; remove: ReturnType<typeof useDeleteSessionNonParticipation>; studentName: string }) { return <TableRow><TableCell>{studentName}</TableCell><TableCell>{item.reason}</TableCell><TableCell><div className="flex gap-2"><Button disabled={readOnly} onClick={onEdit} size="sm" type="button">Edit</Button><Button disabled={readOnly || remove.isPending} onClick={() => remove.mutate(item.id)} size="sm" type="button" variant="outline">Remove</Button></div></TableCell></TableRow> }
