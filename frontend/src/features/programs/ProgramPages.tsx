...
import {
  ...
  useDeleteSessionNonParticipation, useMeetingDates,
...
function MeetingDateDraftList({ dates, onChange }: { dates: string[]; onChange: (dates: string[]) => void }) {
  ...
  return <div className="space-y-3"><div className="flex gap-2"><Input aria-label="New meeting date" onChange={(event) => setNewDate(event.target.value)} type="date" value={newDate} /><Button disabled={!newDate} onClick={addDate} type="button" variant="outline">Add date</Button></div>{dates.length === 0 && <p className="text-sm text-muted-foreground" role="status">Add at least one meeting date.</p>}{dates.some((date) => !date) && <p className="text-sm text-destructive" role="alert">Each meeting date is required.</p>}<div className="space-y-2">{dates.map((date, index) => <div className="flex items-center gap-2" key={`${date}-${index}`}><Input aria-label={`Meeting date ${index + 1}`} onChange={(event) => onChange(dates.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} required type="date" value={date} /><Button aria-label={`Remove meeting date ${index + 1}`} disabled={dates.length === 1} onClick={() => onChange(dates.filter((_, itemIndex) => itemIndex !== index))} size="sm" type="button" variant="outline">Remove</Button></div>)}</div></div>
}
...
export function ProgramListPage() {
  ...
  return <PageFrame>
    ...
    <div className="mt-2 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-semibold tracking-tight">Programs</h1><p className="mt-2 text-sm text-muted-foreground">Author membership, interest areas, sessions, and the catalog for this school year.</p></div><div className="flex gap-2"><Button disabled={readOnly} onClick={() => { setName(''); setCreateOpen(true) }} type="button">Create program</Button><Button asChild variant="outline"><Link to={`/y/${schoolYearId}/students`}>Open roster</Link></Button></div></div>
...
export function SessionPage() {
  ...
  return <PageFrame>
    ...
    <Card title="Meeting dates" description="Every offering meets on every date listed here."><div className="mt-4 flex items-center justify-between gap-3"><div className="space-y-1">{(dates.data ?? []).map((date) => <p className="text-sm" key={date.id}>{dateLabel(date.meeting_date)} <span className="text-muted-foreground">({date.meeting_date})</span></p>)}</div><Button disabled={readOnly} onClick={openSessionEditor} type="button" variant="outline">Edit dates</Button></div></Card>
