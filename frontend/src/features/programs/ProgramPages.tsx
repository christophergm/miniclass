...
import {
  ...
  useCreateOffering, useCreateProgram, useCreateSession, useCreateSessionNonParticipation,
  useDeleteMeetingDate, useDeleteOffering, useDeleteSessionNonParticipation, useMeetingDates,
  useOfferings, useProgramInterestAreas, useProgramMemberships, useProgramObjectiveWeights,
  ...
  useUpdateInterestArea, useUpdateMeetingDate, useUpdateOffering, useUpdateProgramObjectiveWeights,
...
export function SessionPage() {
  ...
  const createOffering = useCreateOffering(schoolYearId ?? '', programId ?? '', sessionId ?? '')
  const updateOffering = useUpdateOffering(schoolYearId ?? '', programId ?? '', sessionId ?? '')
  const deleteOffering = useDeleteOffering(schoolYearId ?? '', programId ?? '', sessionId ?? '')
  ...
  return <PageFrame>
    ...
    <div className="mt-2 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-semibold tracking-tight">{current.name}</h1><p className="mt-2 text-sm text-muted-foreground">Session {current.ordinal} · {stateLabel(current.state)}</p></div><Link className="text-sm font-medium text-primary hover:underline" to={`/y/${schoolYearId}/programs/${programId}`}>Back to program</Link></div>
...
function OfferingSection({ readOnly, offerings, grades, areas, create, update, remove, editingOffering, setEditingOffering }: { readOnly: boolean; offerings: Offering[]; grades: ReturnType<typeof activeGradeLevels>; areas: InterestArea[]; create: ReturnType<typeof useCreateOffering>; update: ReturnType<typeof useUpdateOffering>; remove: ReturnType<typeof useDeleteOffering>; editingOffering: string | null; setEditingOffering: (id: string | null) => void }) {
  const [value, setValue] = useState({ name: '', description: '', capacity: '1', minimum_viable_enrollment: '', min_grade_level_id: '', max_grade_level_id: '', location: '', meeting_point: '', meeting_instructions: '', interest_area_id: '' })
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); create.mutate({ ...value, capacity: Number(value.capacity), minimum_viable_enrollment: value.minimum_viable_enrollment === '' ? null : Number(value.minimum_viable_enrollment), interest_area_id: value.interest_area_id || null }, { onSuccess: () => setValue({ ...value, name: '', description: '' }) }) }
  const gradesSelect = <><option value="">Choose grade</option>{grades.map((grade) => <option key={grade.id} value={grade.id}>{grade.label}</option>)}</>
  return <Card title="Offerings" description="Capture description, capacity, grade window, minimum viability, location, meeting instructions, and an optional interest area."><form className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3" onSubmit={submit}><Input aria-label="Offering name" disabled={readOnly} onChange={(event) => setValue({ ...value, name: event.target.value })} placeholder="Offering name" value={value.name} /><Input aria-label="Offering description" disabled={readOnly} onChange={(event) => setValue({ ...value, description: event.target.value })} placeholder="Description" value={value.description} /><Input aria-label="Offering capacity" disabled={readOnly} min="1" onChange={(event) => setValue({ ...value, capacity: event.target.value })} type="number" value={value.capacity} /><select aria-label="Minimum grade" className="h-9 rounded-md border bg-transparent px-3 text-sm" disabled={readOnly} onChange={(event) => setValue({ ...value, min_grade_level_id: event.target.value })} value={value.min_grade_level_id}>{gradesSelect}</select><select aria-label="Maximum grade" className="h-9 rounded-md border bg-transparent px-3 text-sm" disabled={readOnly} onChange={(event) => setValue({ ...value, max_grade_level_id: event.target.value })} value={value.max_grade_level_id}>{gradesSelect}</select><Input aria-label="Minimum viable enrollment" disabled={readOnly} min="0" onChange={(event) => setValue({ ...value, minimum_viable_enrollment: event.target.value })} placeholder="Minimum viable enrollment" type="number" value={value.minimum_viable_enrollment} /><Input aria-label="Offering location" disabled={readOnly} onChange={(event) => setValue({ ...value, location: event.target.value })} placeholder="Location" value={value.location} /><Input aria-label="Meeting point" disabled={readOnly} onChange={(event) => setValue({ ...value, meeting_point: event.target.value })} placeholder="Meeting point" value={value.meeting_point} /><Input aria-label="Meeting instructions" disabled={readOnly} onChange={(event) => setValue({ ...value, meeting_instructions: event.target.value })} placeholder="Meeting instructions" value={value.meeting_instructions} /><select aria-label="Interest area" className="h-9 rounded-md border bg-transparent px-3 text-sm" disabled={readOnly} onChange={(event) => setValue({ ...value, interest_area_id: event.target.value })} value={value.interest_area_id}><option value="">No interest area</option>{areas.filter((area) => !area.retired_at).map((area) => <option key={area.id} value={area.id}>{area.label}</option>)}</select><Button disabled={readOnly || create.isPending || !value.name.trim() || !value.min_grade_level_id || !value.max_grade_level_id} type="submit">Create offering</Button></form>{create.isError && <Problem error={create.error} fallback="Unable to create the offering." />}<div className="mt-6 space-y-3">{offerings.map((offering) => editingOffering === offering.id ? <OfferingEdit key={offering.id} offering={offering} grades={grades} areas={areas} readOnly={readOnly} update={update} cancel={() => setEditingOffering(null)} /> : <div className="rounded-md border p-4" key={offering.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-medium">{offering.name}</h3><p className="mt-1 text-sm text-muted-foreground">{offering.description || 'No description'} · capacity {offering.capacity} · {offering.location || 'Location not set'}</p><p className="mt-1 text-xs text-muted-foreground">Grade window {grades.find((grade) => grade.id === offering.min_grade_level_id)?.label ?? offering.min_grade_level_id}–{grades.find((grade) => grade.id === offering.max_grade_level_id)?.label ?? offering.max_grade_level_id}</p></div><div className="flex gap-2"><Button disabled={readOnly} onClick={() => setEditingOffering(offering.id)} size="sm" type="button" variant="outline">Edit</Button><Button disabled={readOnly || remove.isPending} onClick={() => remove.mutate(offering.id)} size="sm" type="button" variant="outline">Delete</Button></div></div></div>)}</div></Card>
}

function OfferingEdit({ offering, grades, areas, readOnly, update, cancel }: { offering: Offering; grades: ReturnType<typeof activeGradeLevels>; areas: InterestArea[]; readOnly: boolean; update: ReturnType<typeof useUpdateOffering>; cancel: () => void }) { const [value, setValue] = useState({ name: offering.name, description: offering.description, capacity: String(offering.capacity), minimum_viable_enrollment: offering.minimum_viable_enrollment == null ? '' : String(offering.minimum_viable_enrollment), min_grade_level_id: offering.min_grade_level_id, max_grade_level_id: offering.max_grade_level_id, location: offering.location, meeting_point: offering.meeting_point, meeting_instructions: offering.meeting_instructions, interest_area_id: offering.interest_area_id ?? '' }); const options = (placeholder: string) => <><option value="">{placeholder}</option>{grades.map((grade) => <option key={grade.id} value={grade.id}>{grade.label}</option>)}</>; return <form className="grid gap-3 rounded-md border border-primary/40 p-4 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); update.mutate({ offeringID: offering.id, value: { ...value, capacity: Number(value.capacity), minimum_viable_enrollment: value.minimum_viable_enrollment === '' ? null : Number(value.minimum_viable_enrollment), interest_area_id: value.interest_area_id || null } }, { onSuccess: cancel }) }}><Input aria-label="Edit offering name" disabled={readOnly} onChange={(event) => setValue({ ...value, name: event.target.value })} value={value.name} /><Input aria-label="Edit offering description" disabled={readOnly} onChange={(event) => setValue({ ...value, description: event.target.value })} value={value.description} /><Input aria-label="Edit offering capacity" disabled={readOnly} min="1" onChange={(event) => setValue({ ...value, capacity: event.target.value })} type="number" value={value.capacity} /><select aria-label="Edit minimum grade" className="h-9 rounded-md border bg-transparent px-3 text-sm" disabled={readOnly} onChange={(event) => setValue({ ...value, min_grade_level_id: event.target.value })} value={value.min_grade_level_id}>{options('Choose minimum grade')}</select><select aria-label="Edit maximum grade" className="h-9 rounded-md border bg-transparent px-3 text-sm" disabled={readOnly} onChange={(event) => setValue({ ...value, max_grade_level_id: event.target.value })} value={value.max_grade_level_id}>{options('Choose maximum grade')}</select><Input aria-label="Edit minimum viable enrollment" disabled={readOnly} min="0" onChange={(event) => setValue({ ...value, minimum_viable_enrollment: event.target.value })} type="number" value={value.minimum_viable_enrollment} /><Input aria-label="Edit offering location" disabled={readOnly} onChange={(event) => setValue({ ...value, location: event.target.value })} value={value.location} /><Input aria-label="Edit meeting point" disabled={readOnly} onChange={(event) => setValue({ ...value, meeting_point: event.target.value })} value={value.meeting_point} /><Input aria-label="Edit meeting instructions" disabled={readOnly} onChange={(event) => setValue({ ...value, meeting_instructions: event.target.value })} value={value.meeting_instructions} /><select aria-label="Edit interest area" className="h-9 rounded-md border bg-transparent px-3 text-sm" disabled={readOnly} onChange={(event) => setValue({ ...value, interest_area_id: event.target.value })} value={value.interest_area_id}><option value="">No interest area</option>{areas.filter((area) => !area.retired_at).map((area) => <option key={area.id} value={area.id}>{area.label}</option>)}</select><div className="flex gap-2"><Button disabled={readOnly || update.isPending} type="submit">Save offering</Button><Button onClick={cancel} type="button" variant="outline">Cancel</Button></div>{update.isError && <Problem error={update.error} fallback="Unable to update the offering." />}</form> }
