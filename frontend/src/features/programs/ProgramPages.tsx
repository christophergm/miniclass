...
import { useAddProgramMembership, useCreateInterestArea, useCreateProgram, useMissingGradeCount, useProgramInterestAreas, useProgramMemberships, usePrograms, useRemoveProgramMembership, useReorderInterestAreas, useUpdateInterestArea } from './usePrograms'
...
export function ProgramMembershipPage() {
  ...
  const reorderInterestAreas = useReorderInterestAreas(schoolYearId ?? '', programId ?? '')
  const [studentID, setStudentID] = useState('')
  const [interestAreaLabel, setInterestAreaLabel] = useState('')
  const orderedInterestAreas = [...(interestAreas.data ?? [])].sort((a, b) => a.ordinal - b.ordinal)
  function moveInterestArea(index: number, direction: -1 | 1) {
    const next = [...orderedInterestAreas]
    const target = index + direction
    if (target < 0 || target >= next.length) return
    const [area] = next.splice(index, 1)
    next.splice(target, 0, area)
    reorderInterestAreas.mutate(next.map((item) => item.id))
  }
