...
const owner: MeResponse = {
  role: "owner",
  principal: { id: "user-1", email: "owner@example.com" },
...
const entry: AuditLogResponse["entries"] extends (infer Entry)[] | null ? Entry : never = {
  ...
  actor: { type: "user", label: "Ada Organizer" },
  action: "school_year_create",
  object_type: "school_year",
  object_id: "year-1",
  change_summary: { name: "2026–2027", state: "setup" },
  reason: "Annual setup",
};
...
describe("AuditLog", () => {
  ...
  it("renders readable entries and requests the next page with the cursor", async () => {
    ...
    vi.mocked(resourceApi.getAuditLog).mockResolvedValue({
      entries: [entry],
    ...
    await waitFor(() =>
      expect(resourceApi.getAuditLog).toHaveBeenLastCalledWith({ cursor: "next-page" }),
    );
  });
});
