...
export function AuditLog() {
  ...
  if (audit.isLoading) {
    return (
      <main
        className="mx-auto flex min-h-screen w-full max-w-5xl items-center justify-center px-6 py-12"
        role="status"
      >
        Loading audit log…
      </main>
  ...
  if (audit.isError || !audit.data) {
    const message =
      audit.error instanceof ApiError ? audit.error.message : "Unable to load the audit log.";
    return (
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-5xl px-6 py-12">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            A history of significant changes to this organization.
          </p>
        </div>
        <form
          ...
          onSubmit={(event) => {
            ...
            setCursor(undefined);
            setAppliedObjectType(objectType.trim());
          }}
        >
          <label className="text-sm font-medium" htmlFor="object-type">
            Object type
            <input
              id="object-type"
              className="mt-1 block h-9 w-44 rounded-md border bg-background px-3 text-sm"
              value={objectType}
      ...
      <section className="mt-8 rounded-lg border bg-card p-2 shadow-sm">
            ...
            {entries.length === 0 ? (
              ...
              entries.map((entry) => (
                <TableRow key={entry.id}>
                  ...
                  <TableCell>{entry.actor.label}</TableCell>
                  <TableCell>{entry.action.replace(/_/g, " ")}</TableCell>
                  <TableCell>{objectLabel(entry)}</TableCell>
                  <TableCell>{readable(entry.change_summary) || "No summary provided"}</TableCell>
