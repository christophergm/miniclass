...
vi.mock("@/features/settings/useSettings", () => ({
  useVocabularyMutation: vi.fn(
    (_schoolYearId: string | undefined, mutationFn: (value: never) => Promise<unknown>) => ({
      mutate: mutationFn,
      isPending: false,
      isError: false,
      error: null,
...
const year = (state: SchoolYear["state"] = "active"): SchoolYear => ({
  id: "year-1",
...
describe("VocabularyPage", () => {
  ...
  it("explains how to finish setup when a year has no vocabulary", () => {
    vi.mocked(useVocabulary).mockReturnValue({
      data: { ...vocabulary, grade_levels: [], homerooms: [] },
      isLoading: false,
      isError: false,
      error: null,
    } as unknown as ReturnType<typeof useVocabulary>);

    renderVocabulary();

    expect(
      screen.getByRole("heading", { name: "Finish setting up this school year" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/required before you can add or import roster records/),
    ).toBeInTheDocument();
  });
});
