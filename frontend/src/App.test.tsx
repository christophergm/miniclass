...
describe("App routing", () => {
  ...
  it("renders a clean not-found page for a foreign school-year URL", async () => {
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        if (url.includes("/api/school-years/foreign-year")) {
          return new Response(
            JSON.stringify({ type: "resource-not-found", detail: "school year not found" }),
    ...
    expect(
      await screen.findByRole("heading", { name: "School year not found" }),
    ).toBeInTheDocument();
    await waitFor(() =>
      expect(screen.getByRole("link", { name: "Back to school years" })).toHaveAttribute(
        "href",
        "/years",
      ),
    );
  ...
  it("reports an unknown address inside a school year without offering its lifecycle controls", async () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "School year not found" }),
...
describe("invitation claim route", () => {
  it("accepts the URL shape the backend generates", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Claim your administrator invitation" }),
    ).toBeInTheDocument();
  });
  ...
  it("explains a link that arrives with no token instead of asking for a password", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "This invitation link is incomplete" }),
    ).toBeInTheDocument();
