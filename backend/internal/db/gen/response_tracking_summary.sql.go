...
// versions:
//   sqlc v1.31.1
// source: response_tracking_summary.sql
...
const listResponseTrackingSummaries = `-- name: ListResponseTrackingSummaries :many
...
type ListResponseTrackingSummariesRow struct {
	...
	InstrumentID      ids.XID `json:"instrument_id"`
	InstrumentName    string  `json:"instrument_name"`
	State             string  `json:"state"`
	SchoolYearID      ids.XID `json:"school_year_id"`
	ProgramID         ids.XID `json:"program_id"`
	TotalStudents     int32   `json:"total_students"`
	RespondedStudents int32   `json:"responded_students"`
}
...
func (q *Queries) ListResponseTrackingSummaries(ctx context.Context, arg ListResponseTrackingSummariesParams) ([]ListResponseTrackingSummariesRow, error) {
	...
	defer rows.Close()
	items := []ListResponseTrackingSummariesRow{}
	for rows.Next() {
		var i ListResponseTrackingSummariesRow
		if err := rows.Scan(
			&i.InstrumentType,
			&i.InstrumentID,
			&i.InstrumentName,
